/// @description draw_lightning_hue(path,segment length,width,hue1,hue2,alpha,fork path,attractive object, enable particles,weighting)
/// @param path
...
/// @param weighting
function draw_lightning_hue(path, segment, width, hue1, hue2, alpha, fork, attractive, enable, weighting) {
	/*
	By Ground Effect Games
	basically the same as path lightning but with the ability to choose a color range.
	hue1 and hue2 are the colors at each end of the lightning. (0-255)
	...
	*/
	...
	{
	    var seglength,xprev,yprev,xx,yy,x1,y1,x2,y2,offset,dir,forkwidth,forkhue,forkalpha,hue,i,hdelta,color;
	    ...
	    repeat(2)
	    {
	        ...
	        for (i=1; i>0; i-=seglength;)
	        {
	            ...
	            alpha=alpha*(1-i/4);
	            xx = path_get_x(path,i-seglength);
	            yy = path_get_y(path,i-seglength);
	    ...
	    if (forkwidth)
	    {
	        if (forkwidth>6)
	        {
	            ...
	            draw_lightning_hue(fork,segment,forkwidth,forkhue,hue2,forkalpha,-1,-1,enable*(forkwidth>12),0);
	        }
	        else
	        {
	            draw_lightning_forked(x1,y1,x2,y2,segment,merge_color(c_white,make_color_hsv(forkhue,255,255),.3),30+random(30),fx.curl,attractive);
	        }
	    }
	}
