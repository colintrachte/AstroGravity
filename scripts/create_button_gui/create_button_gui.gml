/// @description create_button_gui(x,y,target x,target y,width,target width,height,target height,sprite_alpha,target sprite_alpha,image_alpha,target image_alpha,loading bar %,target loading bar %,string label,click action #)
/// @param x
/// @param y
/// @param target x
/// @param target y
/// @param width
/// @param target width
/// @param height
/// @param target height
/// @param sprite_alpha
/// @param target sprite_alpha
/// @param image_alpha
/// @param target image_alpha
/// @param loading bar %
/// @param target loading bar %
/// @param string label
/// @param click action #
function create_button_gui(argument0, argument1, argument2, argument3, argument4, argument5, argument6, argument7, argument8, argument9, argument10, argument11, argument12, argument13, argument14, argument15) {
	//creates a default button example
	button=instance_create(argument0,argument1,o_button);
	with button
	{
	    parent_id=other.id;
	    selected=0;
	    margin=8;
	    image_speed=0;
    
	    w=argument4; h=argument6;
	    tw=argument5; th=argument7;
    
	    x1=x-w; y1=y-h;//top right corner
	    x2=x+w; y2=y+h;//bottom left corner
	    tx=argument2;   ty=argument3;
    
	    activated=1;//determines whether or not to update
	    touch=0;
	    hover=0;
	    update_speed=3;
	    alarm[0]=update_speed;
    
	    alpha=argument8;
	    target_alpha=argument9;
	    image_alpha=argument10;
	    target_image_alpha=argument11;
    
	    //loading bar
	    q=argument12;
	    tq=argument13;
	    color=make_color_hsv(q*64,255*activated,255);
    
	    label=argument14;
	    click_action=argument15;
    
	    //after all your other variables are set, initialize the grid
	    holygrid=ds_grid_create(6,7);
	    scr_holygrid_preset(holygrid,0);
	}
	return button;



}
