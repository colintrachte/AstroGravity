/// @description draw_lightning_path(path,segment length,starting width,color,alpha,fork path,attractive object, enable particles,weighting)
/// @param path
...
/// @param weighting
function draw_lightning_path(path, segment, starting, color, alpha, fork, attractive, enable, weighting) {
	/*
	By Ground Effect Games
	regular path-based lightning
	...
	*/
	...
	{
	    var seglength,width,xprev,yprev,xx,yy,x1,y1,x2,y2,offset,dir,dir2,forkwidth,forkalpha,i,i2;
	    ...
	    if (forkwidth)
	    {
	        ...
	        if (forkwidth>6)
	        {
	            ...
	            draw_lightning_path(fork,segment,forkwidth,color,forkalpha,-1,-1,enable*(forkwidth>12),0);
	        }
	        else
	        {
	            draw_lightning_forked(x1,y1,x2,y2,segment,merge_color(c_white,color,.3),30+random(30),fx.curl,enable);
	        }
	    }
	}
